exports.commands = function (room, chatter, message, self, reportError) {
    ...
    if ((message.toLowerCase()).includes("!drop")) {
        TwitchClient.say(room, chatter['display-name'] + ' was placed #' + parseInt((Math.random() * 100) + 1) + ' after fighting in the arena')
    }
    ...
    if ((message.toLowerCase()).split(' ')[0].includes("!hug") && message.toLowerCase().split(' ')[1]) {
        TwitchClient.action(room, chatter['display-name'] + ' hugged ' + message.toLowerCase().split(' ')[1] + '! ...Awh that\'s so sweet! ... I think I\'m going to cry... (づ╥ᆺ╥)づ Wai u do dis?!')
    }
    ...
    if ((message.toLowerCase()).includes("!nox")) {
        TwitchClient.action(room, '" I’m a guy! I’m a guys! Oh fuck that shit I’m a Nox" – NoxGamingQC, 2017-12-17')
    }
